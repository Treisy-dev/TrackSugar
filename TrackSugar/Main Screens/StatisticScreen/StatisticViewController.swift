...
final class StatisticViewController: UIViewController {
    ...
    init(viewModel: StatisticViewModelProtocol) {
        ...
        let sugarAnalysis = viewModel.getSugarAnalysis()
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        contentView?.checkExportButtonState()
    }
    ...
    private func updateData(startDate: Date, endDate: Date, for duration: DateDuration) {
        ...
        viewModel.updateDataSource(startDate: startDate, endDate: endDate)
        NotificationCenter.default.post(name: Notification.Name("updateStatisticDataNotification"), object: (startDate, endDate))
        if viewModel.dataSource.count == 0 {
            contentView?.scrollAddition = 175 // Уменьшаем для новой высоты
        } else {
            contentView?.scrollAddition = viewModel.dataSource.count >= 7 ? -30 : CGFloat(215 - 57 * viewModel.dataSource.count)
        }
    }
    ...
    private func setUpExportButton() {
        contentView?.exportButton.addTarget(self, action: #selector(exportButtonTapped), for: .touchUpInside)
    }
    ...
    @objc private func exportButtonTapped() {
        ...
        guard let csvURL = viewModel.exportToCSV() else {
            ...
            showAlert(title: "Ошибка", message: "Нет данных для экспорта")
            return
        }
        
        print("CSV file created at: \(csvURL)")
        ...
        if let popoverController = activityViewController.popoverPresentationController {
            popoverController.sourceView = contentView?.exportButton
            popoverController.sourceRect = contentView?.exportButton.bounds ?? .zero
        }
